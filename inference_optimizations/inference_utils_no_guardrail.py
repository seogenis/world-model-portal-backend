...
def process_prompt(
    ...
    This modified version disables guardrails for speed optimization.
    ...
    print_rank_0("Guardrails disabled for speed optimization")

    if enable_prompt_upsampler and prompt_upsampler_dir:
        if image_path:
            print_rank_0("Prompt upsampling is not supported for image generation, using original prompt")
            return prompt
        else:
            try:
                ...
                print_rank_0(f"Error in prompt upsampling: {e}, using original prompt")
                return prompt
    else:
        return prompt
...
def save_video(
    ...
    """
    Save video frames to file without safety checks for speed optimization.
    ...
    Args:
        ...
        guardrails_dir (str): Directory containing guardrails weights.
    """
    # No video guardrails applied for speed optimization
    print_rank_0("Video guardrails disabled for speed optimization")
